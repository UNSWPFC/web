const IntroductionToInvesting2023 = {
  title: "Introduction to Investing (2023)",
  date: "9th July 2023",
  description:
    "Welcome to our 'Introduction to Investing' newsletter series!\
    Before we get started discussing how to set up an account or different stocks,\
    it's important to first understand what personal finance really is, and how investing can help you achieve your goals!",
  nav: "introduction-to-investing-2023",
  images: [
    "introduction-to-investing-2023-1.jpg",
    "introduction-to-investing-2023-2.png",
    "introduction-to-investing-2023-3.png",
    "introduction-to-investing-2023-4.png",
    "introduction-to-investing-2023-5.png",
    "introduction-to-investing-2023-6.png",
    "introduction-to-investing-2023-7.png",
  ],
  cover: "Introduction-to-investing-2023-cover.webp",
};

const IntroductionToSaving = {
  title: "Introduction to Saving",
  date: "1st March 2021",
  description:
    "This week we are covering some budgeting tips and ways to reduce unnecessary spending.",
  nav: "introduction-to-saving",
  images: ["introduction-to-saving-1.png", "introduction-to-saving-2.png"],
  cover: "introduction-to-saving-cover.webp",
};

const IntroductionToInvesting2021 = {
  title: "Introduction to Investing (2021)",
  date: "15th February 2021",
  description:
    "Welcome to the first official UNSW PFC article post of 2021.\
    We will be writing and producing content to bring new insights into your own personal finances.\
    Also, keep an eye out as subcommittee recruitment applications will be out soon!",
  nav: "introduction-to-investing-2021",
  images: [
    "introduction-to-investing-2021-1.png",
    "introduction-to-investing-2021-2.png",
    "introduction-to-investing-2021-3.png",
  ],
  cover: "introduction-to-investing-2021-cover.jpg",
};

const HolidayBudgeting = {
  title: "Holiday Budgeting",
  date: "27th December 2020",
  description:
    "Been spending more than you should recently? Want to learn some tips on saving money with your purchases?\
    Amidst the holiday season, it's often difficult to not add things to your cart with the overwhelming sales everywhere we go.\
    In collaboration with UNSW Finance and Banking Society,\
    we have prepared some helpful tips to protect our wallets and meet our financial goals!",
  nav: "holiday-budgeting",
  images: ["holiday-budgeting-1.png", "holiday-budgeting-2.jpeg"],
  cover: "holiday-budgeting-cover.png",
};

export const PublicationInfo = [
  IntroductionToInvesting2023,
  IntroductionToSaving,
  IntroductionToInvesting2021,
  HolidayBudgeting,
];
